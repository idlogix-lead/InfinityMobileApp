...
const SignIn = ({ navigation }) => {
    ...
    const login = async () => {
        ...
        try {
            // Attempt to login
            const loginResponse = await fetch(`${protocol}://${host}:${port}/api/v1/auth/tokens`, {
                method: 'POST',
            ...
            navigation.navigate(userName.trim() === getPreUser ? 'FingerPrintScreen' : 'SelectRoleScreen', { token: sessionData.token, tokenOk: sessionData.userId, roleId: await AsyncStorage.getItem('roleId') });
        } catch (error) {
            alert(error.message);
        } finally {
            ...
            {!isLoading && (
                <View style={{ flex: 1,
                    ...
                    <View style={styles.inpucontainer}>
                        <View>
                        <Text style={{ marginTop: "5%", fontSize: 20, fontWeight: "800", color: "black", paddingLeft: "3%"}}>Welcome</Text>
                        <Text style={{color:"gray",  paddingLeft: "3%", fontSize:16}}>Login to access your account</Text>
                                ...
                                <CheckBox
                                    ...
                                    checkedIcon={
                                        <View style={{ height: 30, width: 30 }}>
                                            <Ionicons name='checkmark-sharp' color='black' size={30} />
                                        </View>
                                    }
                                    uncheckedIcon={
