...
const CrmExpire = ({route, navigation}) => {
  const {expireLeads} = route.params;

  // todayLeadsCount
  const todayLeadsCount = expireLeads
    ? expireLeads.filter(lead => moment(lead.Created).isSame(moment(), 'day'))
        .length
    : 0;

  // monthsLeadsCount
  const monthLeadsCount = expireLeads
    ? expireLeads.filter(lead => moment(lead.Created).isSame(moment(), 'month'))
        .length
    : 0;
  ...
  return (
    <Provider>
      <View style={{flex: 1}}>
        <CustomHeader title={'Expire Leads'} />
        <View style={styles.main}>
          <View
            ...
            <View style={{marginLeft: 20, marginTop: 20}}>
              ...
              <Text style={styles.filterTxt}>{expireLeads.length}</Text>
            </View>
          </View>
          <FlatList
            data={expireLeads
