...
const CrmNew = ({route, navigation}) => {
  const {newLeads, activity} = route.params;

  // Today Leads
  const todayLeadsCount = newLeads
    ? newLeads.filter(lead => moment(lead.Created).isSame(moment(), 'day'))
        .length
    : 0;

  // This Month Leads
  const monthLeadsCount = newLeads
    ? newLeads.filter(lead => moment(lead.Created).isSame(moment(), 'month'))
        .length
    : 0;

  // ✅ Conversion Calculation
  const totalLeads = newLeads ? newLeads.length : 0;

  const convertedLeads = newLeads
    ? newLeads.filter(lead => lead.LeadStatus?.identifier === 'Converted')
        .length
    : 0;
        ...
        <View style={styles.main}>
          ...
          <View
            ...
            <View style={{marginLeft: 20, marginTop: 20}}>
              ...
              <Text style={styles.filterTxt}>{newLeads.length}</Text>
            </View>
          </View>
          {/* Card */}
          <FlatList
