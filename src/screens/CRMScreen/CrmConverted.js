...
const CrmConverted = ({route, navigation}) => {
  const {convertedLeads, activity} = route.params;

  // todayLeadsCount
  const todayLeadsCount = convertedLeads
    ? convertedLeads.filter(lead =>
  ...
  const monthleadsCount = convertedLeads
    ? convertedLeads.filter(lead =>
        moment(lead.Created).isSame(moment(), 'day'),
      ).length
    : 0;
        ...
        <View style={styles.main}>
          <View
            ...
            <View style={{marginLeft: 20, marginTop: 20}}>
              ...
              <Text style={styles.filterTxt}>{convertedLeads.length}</Text>
            </View>
          </View>
          ...
          <FlatList
            ...
            renderItem={({item}) => {
              ...
              const userActivity = activity.filter(
                act => act => act?.AD_User_ID?.id === item?.id,
              );
              // console.log('converted Activities', userActivity);
             ...
             // Sort by Created date (assuming it's act.Created)
              ...
              const activityLength = userActivity.length;
              return (
                <CRMCard
