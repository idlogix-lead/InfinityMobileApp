...
const AnnualLeave = ({ navigation, route }) => {
    ...
    const getYearId = async () => {
        ...
        const fiscalYearFormatCurrent = `${currentYear}/${(currentYear + 1).toString().slice(-2)}`;
        const fiscalYearFormatPrevious = `${previousYear}/${(previousYear + 1).toString().slice(-2)}`;
        ...
        try {
            ...
            const filteredData = response.data.records.filter(item =>
                item.FiscalYear === fiscalYearFormatCurrent || item.FiscalYear === fiscalYearFormatPrevious
            );
                ...
                <View style={{ marginBottom: '7%' }}>
                    ...
                    <View style={styles.startDate}>
                        ...
                        <View>
                            {years.length > 0 ? (
                                years.map((year, index) => (
                                    <TouchableOpacity key={index} onPress={() => handleYearSelection(year)}>
                                        <Text style={{ color: 'gray', paddingLeft: "2%" }}>{year.label}</Text>
                                    </TouchableOpacity>
                                ))
                            ) : (
                                <Text>No fiscal year data available</Text>
