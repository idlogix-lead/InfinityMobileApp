...
const CreateNewReq = ({navigation}) => {
  ...
  const getAPIData = async (protocol, host, port, userId) => {
    ...
    fetch(
      `${protocol}://${host}:${port}/api/v1/models/AD_User?$filter=SalesRep_ID eq ${userId}`,
      {
        method: 'GET',
        headers: {
      ...
      .then(data => {
        const array1 = [
          {Name: 'Select SubOrdinate', Name: 'Select SubOrdinate'},
          {Name: value, Name: value, id: userId},
        ];
  ...
  const saveData = async () => {
    try {
      ...
      const orgResponse = await fetch(
        `${protocol}://${host}:${port}/api/v1/auth/organizations?client=${clientId}&role=${roleId}`,
        {
          method: 'GET',
          headers: {
            'Content-Type': 'application/json',
      ...
      const AD_Org_ID = orgData.organizations.find(
      ...
      const payload = {
        ...
        R_RequestType_ID: {
          id: 1000000,
          identifier: 'Personal Tasks',
          'model-name': 'r_requesttype',
          propertyLabel: 'Request Type',
        },
        R_Status_ID: {
          id: 1000000,
          identifier: '9_open',
      ...
      const attachmentResponse = await fetch(
        `${protocol}://${host}:${port}/api/v1/models/R_Request/${newDocId}/attachments`,
        {
          method: 'POST',
  ...
  const Save = async () => {
    ...
    // }
      ...
      endDate.toISOString().substring(0, 10) < formattedDate.substring(0, 10)
    ) {
                ...
                <FlatList
                  ...
                  renderItem={({item}) => (
                    <TouchableOpacity
                      ...
                      <Text style={[styles.topTxt, {alignSelf: 'center'}]}>
                        {item.Name}
                      </Text>
                    </TouchableOpacity>
                  )}
        ...
        <View
          ...
          <TouchableOpacity style={styles.btnSave} onPress={() => Save()}>
            <MaterialCommunityIcons
              ...
              color="gray"
            />
            <Text style={[styles.topTxt, {marginLeft: 5}]}>Create</Text>
          </TouchableOpacity>
        </View>
      </View>
