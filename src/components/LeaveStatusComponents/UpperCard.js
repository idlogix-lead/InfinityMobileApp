...
const UpperCard = () => {
  ...
  const LeaveResponseApi = async () => {
    ...
    try {
      ...
      const url = `${protocol}://${host}:${port}/api/v1/models/HR_EmpLev_Bal?$filter=C_BPartner_ID eq ${partnerId}`;
      console.log(url,'ForLeaveURL')
      ...
      // console.log(response?.data?.records,'UpperCardAPIGET')
