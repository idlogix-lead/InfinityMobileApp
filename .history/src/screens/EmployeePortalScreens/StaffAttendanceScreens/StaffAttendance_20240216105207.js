...
const StaffAttendance = () => {
    ...
    const onDepartmentSuggestionSelect = async (selectedDepartment, selectedDepartmentId) => {
        ...
        const host = await AsyncStorage.getItem('host');
        const port = await AsyncStorage.getItem('port');
        const url = `${protocol}://${host}:${port}/api/v1/path-to-fetch-subdepartments?departmentId=${selectedDepartmentId}`;
        ...
        try {
            ...
            // Assuming the response contains an array of subdepartments
            setSubDepartmentSuggestions(response.data.records);
        } catch (error) {
