...
const StaffAttendance = () => {
    ...
    const buildFilterParams = () => {
        ...
        if (searchSubDepartment) params.subDepartment = searchSubDepartment;
        return params;
    };
    ...
    const getProcessedAttandance = async () => {
        ...
        if (searchDepartment) filterConditions.push(`department eq '${searchDepartment}'`);
        if (searchQuery) filterConditions.push(`business_partner eq '${searchQuery}'`);
        if (searchSubDepartment) filterConditions.push(`subdepartment eq '${searchSubDepartment}'`);
        
        // Convert Unix timestamps to ISO 8601 format and append to filter conditions
        if (selectedStartDate && selectedEndDate) {
            const startDateIso = new Date(parseInt(selectedStartDate)).toISOString();
            const endDateIso = new Date(parseInt(selectedEndDate)).toISOString();
            filterConditions.push(`AttDate ge ${startDateIso} and AttDate le ${endDateIso}`);
        }
    
        // Join the conditions with 'and' if there are any
        let filterQuery = '';
        if (filterConditions.length > 0) {
            filterQuery = `$filter=${encodeURIComponent(filterConditions.join(' and '))}`;
        }
    
        const queryParams = [filterQuery, `$top=100`, `$skip=0`].filter(Boolean).join('&');
        const apiUrl = `${protocol}://${host}:${port}/api/v1/models/HR_Processed_Attendance_V?${queryParams}`;
        ...
        try {
            ...
            // Assuming getCurrentMonthRecords is a function that filters records based on the current month
            const currentMonthData = getCurrentMonthRecords(response.data.records);
            setCurrentMonthProcessed(currentMonthData);
        } catch (error) {
            console.error('Error fetching processed attendance', error);
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const getUnProcessedAttandance = async () => {
        ...
        try {
            const response = await axios.get(`${protocol}://${host}:${port}/api/v1/models/HR_UnProcessed_Attendance_V?${queryParams}`,
                {
                    method: 'GET',
                    headers: {
                        'Content-Type': 'application/json',
    ...
    const getEmployeeName = async () => {
        ...
        const port = await AsyncStorage.getItem('port')
        try {
            const response = await axios.get(`${protocol}://${host}:${port}/api/v1/models/C_BPartner?$top=100`,
                {
                    method: 'GET',
                    headers: {
                        'Content-Type': 'application/json',
    ...
    }, [searchQuery, selectedStartDate, selectedEndDate, searchDepartment, searchSubDepartment])
