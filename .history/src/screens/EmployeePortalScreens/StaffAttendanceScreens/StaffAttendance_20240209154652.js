...
const StaffAttendance = () => {
    ...
    const getProcessedAttandance = async () => {
        ...
        const queryParams = new URLSearchParams({ ...filterParams, $top: calculateTopValue(filterParams), $skip: 0 }).toString();

        try {
            const response = await axios.get(`${protocol}://${host}:${port}/api/v1/models/HR_Processed_Attendance_V?${queryParams}&$top=100&$skip=115000`,
                {
                    method: 'GET',
                    headers: {
    ...
    const calculateTopValue = (filterParams) => {
        // Placeholder function to calculate `$top` based on your filtering logic.
        // You might want to adjust this based on the specific filters applied or other criteria.
        // For example, if a complete month is selected, calculate the expected number of records.
        return 1000; // Example fixed value, adjust as needed.
    };
