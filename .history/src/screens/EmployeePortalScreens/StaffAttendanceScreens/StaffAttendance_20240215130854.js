        ...
        if (query.length >= 3) {
            ...
            try {
                const response = await axios.get(
                    `${protocol}://${host}:${port}/api/v1/models/C_BPartner?$select=Name,IsEmployee`,
                    {
                        method: 'GET',
                        headers: {
                            'Content-Type': 'application/json',
                ...
                const filteredResults = response.data.records.filter(item =>
                    item.IsEmployee === true && item.Name.includes(query)
                );
                
                setEmployeeSuggestions(filteredResults);
            } catch (error) {
                console.log('API Error:', error.response.data);
            }
        } else {
            setEmployeeSuggestions([]); // Clear suggestions if the search query is empty
        }
    };
