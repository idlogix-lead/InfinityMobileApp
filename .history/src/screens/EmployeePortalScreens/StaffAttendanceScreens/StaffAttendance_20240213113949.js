...
const StaffAttendance = () => {
    ...
    const getProcessedAttandance = async (employeeName = '', startDate = '', endDate = '', department = '', subDepartment = '') => {
        ...
        let queryParams = [];
        if (employeeName) queryParams.push(`business_partner eq '${searchQuery}'`);
        if (startDate && endDate) queryParams.push(`AttDate ge '${selectedStartDate}' and AttDate le '${selectedEndDate}'`);
        if (department) queryParams.push(`department eq '${searchDepartment}'`);
        if (subDepartment) queryParams.push(`subdepartment eq '${searchSubDepartment}'`);
