...
const StaffAttendance = () => {
    ...
    let conditions = [];
    if (searchQuery) {
        conditions.push(`business_partner eq '${searchQuery}'`); // Assuming 'searchQuery' doesn't need URI encoding for OData query
    }
    if (searchDepartment) {
        conditions.push(`department eq '${searchDepartment}'`); // Direct inclusion of the string value
    }
    if (searchSubDepartment) {
        conditions.push(`subdepartment eq '${searchSubDepartment}'`); // Direct inclusion of the string value
    }
    if (selectedStartDate && selectedEndDate) {
        conditions.push(`AttDate ge ${selectedStartDate} and AttDate le ${selectedEndDate}`); // Assuming dates are in correct format
    }

    // Joining conditions with ' and ' and NOT encoding them
