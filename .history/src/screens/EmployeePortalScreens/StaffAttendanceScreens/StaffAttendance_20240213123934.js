...
const StaffAttendance = () => {
    ...
    const getProcessedAttandance = async () => {
        ...
        console.log(searchQuery, 'emp')
        console.log(selectedStartDate, 'start')
        console.log(selectedEndDate, 'end')
        console.log(searchDepartment, 'depart')
        console.log(searchSubDepartment, 'subdep')

        // const queryParams = `$top=100&$skip=0&$filter=C_BPartner_ID eq 1001682 and AttDate ge ${selectedStartDate} and AttDate le ${selectedEndDate}`
        const encodedSearchQuery = encodeURIComponent(`'${searchQuery}'`);
        const encodedSearchDepartment = encodeURIComponent(`'${searchDepartment}'`);
        const encodedSearchSubDepartment = encodeURIComponent(`'${searchSubDepartment}'`);
        const encodedSelectedStartDate = encodeURIComponent(`'${selectedStartDate}'`);
        const encodedSelectedEndDate = encodeURIComponent(`'${selectedEndDate}'`);

        // Construct query params with encoded values
        const queryParams = `$filter=business_partner eq ${encodedSearchQuery} and department eq ${encodedSearchDepartment} and subdepartment eq ${encodedSearchSubDepartment} and AttDate ge ${encodedSelectedStartDate} and AttDate le ${encodedSelectedEndDate}`;

        const URL = `${protocol}://${host}:${port}/api/v1/models/HR_Processed_Attendance_V?${queryParams}`
        console.log(URL)
        try {
    ...
    const applyFilters = () => {
        getProcessedAttandance(searchQuery, searchDepartment, searchSubDepartment, selectedStartDate, selectedEndDate)
    }
