...
const StaffAttendance = () => {
    ...
    {/*Empolyee Filter  Api*/ }
    const getEmployeeName = async (searchQuery) => {
        setIsLoading(true);
        const token = await AsyncStorage.getItem('token');
        const protocol = await AsyncStorage.getItem('protocol');
        const host = await AsyncStorage.getItem('host');
        ...
        // Adjust the URL or headers accordingly if your API supports search filtering
        const filterCondition = `business_partner eq '${encodeURIComponent(searchQuery)}'`;
        const URL = `${protocol}://${host}:${port}/api/v1/models/HR_Processed_Attendance_V?$filter=${filterCondition}`;
    ...
    const onChangeSearch = query => {
        ...
        if (query.length >= 3) {
            getEmployeeName(query); // Call the API with the query
        } else {
            setEmployeeSuggestions([]); // Clear suggestions if the query is less than 3 characters
        }
    };
