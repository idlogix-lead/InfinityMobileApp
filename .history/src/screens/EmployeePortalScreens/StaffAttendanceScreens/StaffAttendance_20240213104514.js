...
const StaffAttendance = () => {
    ...
    const getProcessedAttandance = async () => {
        ...
        // Prepare the filter parameters conditionally
        let queryParams = `$top=100&$skip=0`;
    
        // Filter for department, business_partner, and subdepartment
        if (searchDepartment) queryParams += ` and department eq ${searchDepartment}`;
        if (searchQuery) queryParams += ` and business_partner eq ${searchQuery}`;
        if (searchSubDepartment) queryParams += ` and subdepartment eq ${searchSubDepartment}`;
    
        // Filter by Date Range, ensuring both dates are not null
        if (selectedStartDate && selectedEndDate) {
            queryParams += ` and AttDate ge ${selectedStartDate} and AttDate le ${selectedEndDate}`;
        }
    
        const apiUrl = `${protocol}://${host}:${port}/api/v1/models/HR_Processed_Attendance_V?$filter=${encodeURIComponent(queryParams)}`;
        console.log(apiUrl);
