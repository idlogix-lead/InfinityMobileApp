...
const StaffAttendance = () => {
    ...
    const onChangeSearch = async (query) => {
        ...
        if (query.length >= 3) {
            ...
            try {
                const response = await axios.get(`${protocol}://${host}:${port}/api/v1/models/C_BPartner?$select=Name,IsEmployee&$filter=contains(Name, Abdul Rehman) and IsEmployee eq true`,
                    {
                        method: 'GET',
                        headers: {
                            'Content-Type': 'application/json',
