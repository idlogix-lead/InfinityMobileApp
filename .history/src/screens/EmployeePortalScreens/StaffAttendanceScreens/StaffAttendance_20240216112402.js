...
const StaffAttendance = () => {
        ...
        if (query.length >= 3) {
            ...
            try {
                const response = await axios.get(URL,
                    ...
                    console.log(response.data.records,'fdsad')
                setDepartmentSuggestions(response.data.records);
                applyFilters();
            } catch (error) {
                console.log('Error', error)
    ...
    const onDepartmentSuggestionSelect = async (selectedDepartment, selectedDepartmentId) => {
        ...
        const host = await AsyncStorage.getItem('host');
        const port = await AsyncStorage.getItem('port');
        const url = `${protocol}://${host}:${port}/api/v1/models/HR_SubDepartment?$select=Name&id=${selectedDepartmentId}`;
        ...
        try {
            ...
            console.log(response.data.records,'sub')
            const filteredSubDepartments = response.data.records.filter(subDept => 
                subDept.departmentId === selectedDepartmentId);
        
            setSubDepartmentSuggestions(filteredSubDepartments);
        } catch (error) {
