...
const StaffAttendance = () => {
    ...
    {/* For Processed data*/ }
    const getProcessedAttandance = async (employeeName = '', startDate = '', endDate = '', department = '', subDepartment = '') => {
        setIsLoading(true);
        ...
        let queryParams = [];
        if (employeeName) queryParams.push(`business_partner eq '${employeeName}'`);
        if (startDate && endDate) queryParams.push(`AttDate ge '${startDate}' and AttDate le '${endDate}'`);
        if (department) queryParams.push(`department eq '${department}'`);
        if (subDepartment) queryParams.push(`subdepartment eq '${subDepartment}'`);

        const filterQuery = queryParams.length > 0 ? `$filter=${queryParams.join(' and ')}` : '';
        try {
            const response = await axios.get(`${protocol}://${host}:${port}/api/v1/models/HR_Processed_Attendance_V?${filterQuery}`, {
                headers: {
    ...
    useEffect(() => {
        ...
        getProcessedAttandance(searchQuery, selectedStartDate, selectedEndDate, searchDepartment, searchSubDepartment);
    ...
    // For Department
    const onSearchDepartmentChange = query => setSearchDepartment(query);

    // For Sub-Department
    const OnSearchSubDepartment = query => setSearchSubDepartment(query);

    // For Date Change
    const onDateChange = (date, type) => {
        if (type === 'END_DATE') {
            setSelectedEndDate(date.formatTime('YYYY-MM-DD')); // Assuming you use a date library like moment.js for formatting
        } else {
            setSelectedStartDate(date.formatTime('YYYY-MM-DD'));
        }
    };
    const applyFilters = () => {
        getProcessedAttendance(searchQuery, selectedStartDate, selectedEndDate, searchDepartment, searchSubDepartment);
    };
