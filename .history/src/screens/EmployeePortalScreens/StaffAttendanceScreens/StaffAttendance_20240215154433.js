...
const StaffAttendance = () => {
    ...
    const getProcessedAttandance = async (page) => {
        ...
        const newSkip = (page - 1) * 10; // Calculate skip based on the current page
        setSkip(newSkip);

        const URL = `${protocol}://${host}:${port}/api/v1/models/HR_Processed_Attendance_V?${queryParams}&$top=10&$skip=${newSkip}`;
        try {
