...
const StaffAttendance = () => {
    ...
    const onChangeSearch = async (query) => {
        ...
        if (query.length >= 3) {
            try {
                // Call your API to fetch suggestions based on the query
                const response = await axios.get(`${protocol}://${host}:${port}/api/v1/models/C_BPartner?$select=Name,IsEmployee&q=${query}`,
                    {
                        method: 'GET',
                        headers: {
                            'Content-Type': 'application/json',
                ...
                // Update the employee suggestions state with the API response
                const employeeData = response.data.records.filter(item => item.IsEmployee === true);
                setEmployeeSuggestions(employeeData);
            } catch (error) {
            ...
            // If the length of the query is less than 3, clear the suggestions
            setEmployeeSuggestions([]);
        }
    };
