...
const StaffAttendance = () => {
    ...
    const getProcessedAttandance = async () => {
        ...
        // Start building the $filter query string based on selected filters
        let filterConditions = [];
        if (searchQuery) filterConditions.push(`contains(business_partner, '${searchQuery}')`);
        ...
        if (selectedStartDate && selectedEndDate) filterConditions.push(`(AttDate ge ${selectedStartDate} and AttDate le ${selectedEndDate})`);
    
        let queryParams = filterConditions.length > 0 ? `$filter=${filterConditions.join(' and ')}` : '';
