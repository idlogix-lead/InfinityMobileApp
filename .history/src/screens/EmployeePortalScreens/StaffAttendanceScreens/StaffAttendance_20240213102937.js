...
const StaffAttendance = () => {
    ...
    const getProcessedAttandance = async () => {
        ...
        const queryParams = [
            `department eq ${encodeURIComponent(searchDepartment)}`,
            `business_partner eq ${encodeURIComponent(searchQuery)}`,
            `subdepartment eq ${encodeURIComponent(searchSubDepartment)}`,
            `AttDate ge ${formatDate(selectedStartDate)}`,
            `AttDate le ${formatDate(selectedEndDate)}`
        ].join(' and ');
        
        const apiUrl = `${protocol}://${host}:${port}/api/v1/models/HR_Processed_Attendance_V?$filter=${queryParams}&$top=100&$skip=0`;
        // const queryParams = '$filter=department eq '+ searchDepartment + ' and business_partner eq '+ searchQuery + ' and subdepartment eq '+ searchSubDepartment 
        // + ' and AttDate ge ' + selectedStartDate + ' and AttDate le ' + selectedEndDate ;
        // const apiUrl = `${protocol}://${host}:${port}/api/v1/models/HR_Processed_Attendance_V?${queryParams}&$top=100&$skip=0`;
        console.log(apiUrl);
        try {
