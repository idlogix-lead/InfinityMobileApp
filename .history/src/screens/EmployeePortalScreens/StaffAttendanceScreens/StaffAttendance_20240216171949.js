...
const StaffAttendance = () => {
    ...
    const [selectedStartDate, setSelectedStartDate] = useState(getParsedDate(new Date(new Date().getFullYear(), new Date().getMonth(), 1)));
    const [selectedEndDate, setSelectedEndDate] = useState(getParsedDate(new Date()));
    ...
    const getProcessedAttandance = async () => {
        ...
        console.log(selectedStartDate,'st')
        console.log(selectedEndDate,'end')

        let queryParams = `$filter=`;
        let conditions = [];
        if (searchQuery) conditions.push(`business_partner eq '${searchQuery}'`);
        if (searchDepartment) conditions.push(`department eq '${searchDepartment}'`);
        if (searchSubDepartment) conditions.push(`subdepartment eq '${searchSubDepartment}'`);
        if (selectedStartDate && selectedEndDate) conditions.push(`AttDate ge ${selectedStartDate} and AttDate le ${selectedEndDate}`);
    ...
    const getUnProcessedAttandance = async () => {
        ...
        if (searchDepartment) conditions.push(`department eq '${searchDepartment}'`);
        if (searchSubDepartment) conditions.push(`subdepartment eq '${searchSubDepartment}'`);
        if (selectedStartDate && selectedEndDate) conditions.push(`attendance_date ge ${selectedStartDate} and attendance_date le ${selectedEndDate}`);
        queryParams += conditions.join(' and ');
    ...
    const onDepartmentSuggestionSelect = async (selectedDepartment, selectedDepartmentId) => {
