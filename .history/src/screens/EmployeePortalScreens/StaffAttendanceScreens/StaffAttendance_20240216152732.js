...
const StaffAttendance = () => {
    ...
    const getUnProcessedAttandance = async () => {
        ...
        if (searchDepartment) conditions.push(`department eq '${searchDepartment}'`);
        if (searchSubDepartment) conditions.push(`subdepartment eq '${searchSubDepartment}'`);
        if (selectedStartDate && selectedEndDate) conditions.push(`attendance_time ge ${selectedStartDate} and attendance_time le ${selectedEndDate}`);
        queryParams += conditions.join(' and ');

        const skip = (currentPage2 - 1) * page_size;

        try {
            const response = await axios.get(`${protocol}://${host}:${port}/api/v1/models/HR_UnProcessed_Attendance_V?${queryParams}&$top=${page_size}&$skip=${skip}`,
                {
                    method: 'GET',
                    headers: {
                        'Content-Type': 'application/json',
    ...
    const onDateChange = (date, type) => {
        ...
        // Reset pagination and state for a new fetch
        setCurrentPage(1);
        setCurrentPage2(1);
        ...
        setProcessed([]);
        setUnProcessed([]);
        applyFilters(); // Make sure this function can be safely called here
    };

    useEffect(() => {
        if (selectedStartDate && selectedEndDate) {
            applyFilters(); // Ensure this doesn't cause an infinite loop
        }
    ...
    const applyFilters = async () => {
        ...
        // Optionally, reset data arrays if you want to clear old data immediately
        setProcessed([]);
        setUnProcessed([]);
