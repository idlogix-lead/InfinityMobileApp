...
const AnnualLeave = () => {
    ...
    const LeaveRequestInfo = async () => {
        ...
        try {
            ...
            setRecords(response.data.records); // Store the entire response in records
            const extractedData = response.data.records.map(item => ({
                label: item.Name,
                value: item.id
            }));
            setLeaveTypes(extractedData); // Store processed data for Picker
        } catch (error) {
    ...
    const getRefList = async () => {
        ...
        try {
            const response = await axios.get(`${protocol}://${host}:${port}/api/v1/models/AD_Ref_List?$filter=AD_Reference_ID eq 131`, {
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${token}`
                }
    ...
    // const identifiers = data.map(item => item.HR_LevTypes_ID.identifier);
    // const uniqueIdentifiers = identifiers.filter(
    //     (item, index, self) => self.indexOf(item) === index
    // );

    const uniqueYears = Array.from(new Set(data.map(item => item.C_Year_ID.identifier)));
            ...
            const response = await axios.post(`${protocol}://${host}:${port}/api/v1/models/HR_EmpLev_Posting`,
                {
                    ...
                    body: ({
                        ...
                        'AD_Org_ID': { 'id': organizationId, 'identifier': orgName },
                        'C_BPartner_ID': { 'id': partnerId, 'identifier': ptnrName },
                        "IsActive": true,
                ...
                <View style={{ marginBottom: '7%' }}>
                    ...
                    <View style={styles.startDate}>
                        ...
                        >
                            ...
                            {uniqueYears.map((identifier, index) => (
                                <Picker.Item key={index} label={identifier} value={identifier} />
                            ))}
                        </Picker>
                    </View>
