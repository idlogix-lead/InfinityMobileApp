import { ScrollView, StyleSheet, Text, View, TouchableOpacity, Alert, Modal, TextInput } from 'react-native'
...
const AnnualLeave = () => {
    ...
    const [organization, setOrganization] = useState({ id: '', identifier: '' });
    ...
    const FindBusinessPrtId = async () => {
        ...
        try {
            ...
            const orgData = response?.data?.records[0]?.AD_Org_ID;
            if (orgData) {
    ...
    const SaveData = async () => {
        ...
        console.log(selectedLeaveType,'end')
        console.log(selectedYearId,'end')
        console.log(description,'end')
        console.log(organization,'end')
        try {
    ...
    const Save = async () => {
        if (!startDate) {
            ...
            alert('Leave type cannot be null');
        } else if (!selectedYearId) {
            alert('Year cannot be null');
        } else if (description === '') {
            alert('Description cannot be empty');
        } else if (!organization.id || !organization.identifier) {
            alert('Organization cannot be null');
        } else {
            SaveData();
        }
    };
                    ...
                    <View style={styles.startDate}>
                        {organization.id && organization.identifier && (
                            <Picker
                                ...
                                <Picker.Item label={organization.identifier} value={organization.id} />
                            </Picker>
                        )}
                    </View>
