...
const AnnualLeave = () => {
    ...
    const FindBusinessPrtId = async () => {
        ...
        try {
            ...
            setOrganization(response?.data?.records[0]?.AD_Org_ID);
        }
        catch (error) {
                ...
                <View style={{ marginBottom: '7%' }}>
                    ...
                    <View style={styles.startDate}>
                        <Picker
                            style={{ color: 'gray' }}
                            dropdownIconColor={'#0070C0'}
                            selectedValue={organization.id} // Use organization.id as the value
                            onValueChange={(itemValue, itemIndex) => {
                                // Handle value change if needed
                            }}>
                            <Picker.Item label={organization.identifier} value={organization.id} /> {/* Render organization.identifier here */}
                        </Picker>
                    </View>
