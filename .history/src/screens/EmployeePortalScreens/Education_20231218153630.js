    ...
    <ScrollView style={{ flex: 1 }}>
      ...
      {educationData.map((item, index) => (
        <View key={index} style={styles.containerView}>
          ...
          <View style={{ flex: 1 }}>
            <Text style={styles.textTop}>Degree Title: {item.Degree_Title}</Text>
            <Text style={styles.textBottom}>Institute Name: {item.Institute_Name}</Text>
            {/* Additional information like Pass Year can be added here */}
          </View>
        </View>
      ))}
    </ScrollView>
