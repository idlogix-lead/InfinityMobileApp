...
const Education = () => {
  ...
  const getEducationData = async () => {
    ...
    try {
      ...
      console.log('User ID from AsyncStorage:', userId); // Log the user ID
      console.log('First few records:', response.data.records.slice(0, 5)); // Log first few records for inspection
    
      if (Array.isArray(response.data.records)) {
        const userEducationData = response.data.records.filter(item => {
          // Replace 'AD_User_ID' with the correct property name if different
          return item.AD_User_ID && item.AD_User_ID.toString() === userId;
        });
