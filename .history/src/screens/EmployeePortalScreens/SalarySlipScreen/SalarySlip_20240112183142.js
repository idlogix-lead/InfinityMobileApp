...
const SalarySlip = () => {
  ...
  const SalarySlipData = async () => {
    ...
    try {
      ...
      setPickerResponse(response.data.records);
    } catch (error) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      await SalarySlipData();

      // Sort the pickerResponse based on the HR_Period_ID's StartDate
      const sortedResponse = [...pickerResponse].sort((a, b) => {
        const startDateA = new Date(a.StartDate);
        const startDateB = new Date(b.StartDate);
        return startDateB - startDateA;
      });

      // Set the selectedPeriod and selectedPeriodData to the latest month's data
      const latestMonth = sortedResponse[0]?.HR_Period_ID.identifier || '';
      setSelectedPeriod(latestMonth);
      const latestMonthData =
        sortedResponse.find((item) => item.HR_Period_ID.identifier === latestMonth) || {};
      setSelectedPeriodData(latestMonthData);
    };
