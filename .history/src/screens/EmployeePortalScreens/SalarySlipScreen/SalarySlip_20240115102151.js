...
const SalarySlip = () => {
  ...
  const SalarySlipData = async () => {
    ...
    try {
      ...
      twelveMonthsAgo.setHours(0, 0, 0, 0); // Set time to the start of the day
  
      const recentRecords = response.data.records.filter(record => {
        const recordDate = new Date(record.StartDate);
        recordDate.setHours(0, 0, 0, 0); // Set time to the start of the day for comparison
  
        // Compare year and month only
        return (recordDate.getFullYear() > twelveMonthsAgo.getFullYear()) ||
               (recordDate.getFullYear() === twelveMonthsAgo.getFullYear() && recordDate.getMonth() >= twelveMonthsAgo.getMonth());
      });
