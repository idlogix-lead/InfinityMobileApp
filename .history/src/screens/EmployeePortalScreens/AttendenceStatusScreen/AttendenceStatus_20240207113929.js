...
const AttendenceStatus = ({ navigation }) => {
    ...
    let s_date = new Date(new Date().getFullYear(), new Date().getMonth(), 1).toDateString('en-US', { year: 'numeric', month: 'numeric', day: 'numeric' })  ;
    let e_date = new Date();
    let skip_record = 0;
    const page_size = 100;
    ...
    const getTotalAttendence = async () => {
        ...
        try {
            setIsLoading(true);
            const response = await axios.get(`${protocol}://${host}:${port}/api/v1/models/HR_Processed_Attendance_V?$filter=ad_user_id eq ${Id} and att_date ge ${s_date} and att_date le ${e_date}&$skip=${skip_record}&top=${page_size}`,
                {
                    method: 'GET',
                    headers: {
