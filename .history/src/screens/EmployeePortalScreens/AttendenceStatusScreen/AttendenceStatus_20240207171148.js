...
const AttendenceStatus = ({ navigation }) => {
    ...
    const getTotalAttendence =async (startDate, endDate) => {
        ...
        try {
            setIsLoading(true);
            const response = await axios.get(`${protocol}://${host}:${port}/api/v1/models/HR_Processed_Attendance_V?$filter=ad_user_id eq ${Id} and AttDate ge ${startDate} and AttDate le ${endDate}&$skip=${skipRecord}&top=${page_size}`,
                {
                    method: 'GET',
                    headers: {
    ...
    const filterAttendanceRecords = () => {
        setSkipRecord(0); // Assuming you want to reset pagination on filter
    
        let startDate = selectedStartDate;
        let endDate = selectedEndDate || selectedStartDate; // Use the same date if end date is not selected
    
        // Close the bottom sheet before fetching new data
        bottomSheetRef.current.close();
    
        // Now pass the direct values to getTotalAttendence
        getTotalAttendence(startDate, endDate);
    };
