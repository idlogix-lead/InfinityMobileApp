...
const AttendenceStatus = ({ navigation }) => {
    ...
    const getTotalAttendence = async () => {
        ...
        try {
            setIsLoading(true);
            const response = await axios.get(`${protocol}://${host}:${port}/api/v1/models/HR_Processed_Attendance_V?$filter=ad_user_id eq ${Id} and AttDate ge ${s_date} and att_date le ${e_date}&$skip=${skip_record}&top=${page_size}`,
                {
                    method: 'GET',
                    headers: {
