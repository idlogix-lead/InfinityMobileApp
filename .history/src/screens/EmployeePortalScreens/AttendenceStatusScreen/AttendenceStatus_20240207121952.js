...
const AttendenceStatus = ({ navigation }) => {
    ...
    const [startDate, setStartDate] = useState(getParsedDate(new Date(new Date().getFullYear(), new Date().getMonth(), 1) ));
    ...
    // const s_date = getParsedDate(new Date(new Date().getFullYear(), new Date().getMonth(), 1) );
    // const e_date =  getParsedDate(new Date());
    // const skip_record = 0;
    const page_size = 100;
    ...
    const getTotalAttendence = async () => {
        ...
        console.log(s_date,'start')
        console.log(e_date,'end')
        console.log(skip_record,'rec')
        try {
            setIsLoading(true);
            const response = await axios.get(`${protocol}://${host}:${port}/api/v1/models/HR_Processed_Attendance_V?$filter=ad_user_id eq ${Id} and AttDate ge ${s_date} and AttDate le ${e_date}&$skip=${skip_record}&top=${page_size}`,
                {
                    method: 'GET',
                    headers: {
   ...
   function getParsedDate(date){
    ...
    const filterAttendanceRecords = () => {
        ...
        if (selectedStartDate && selectedEndDate) {
            ...
            s_date = new Date(selectedStartDate);
            e_date = new Date(selectedEndDate);

        } else if (selectedStartDate) {
            // Filter for a single date
            s_date = new Date(selectedStartDate);
            e_date = new Date(selectedStartDate);
