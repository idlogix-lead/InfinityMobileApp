...
const AttendenceStatus = ({ navigation }) => {
    ...
    // console.log(firstDayOfMonth.toLocaleString());

    // setSelectedStartDate(firstDayOfMonth.toLocaleString());
    ...
    const getTotalAttendence = async () => {
        ...
        try {
            setIsLoading(true);
            const response = await axios.get(`${protocol}://${host}:${port}/api/v1/models/HR_Processed_Attendance_V?$filter=ad_user_id eq ${Id} and att_date ge ${s_date} and att_date le ${e_date}&$skipe=${skip_record}&top=${page_size}`,
                {
                    method: 'GET',
                    headers: {
