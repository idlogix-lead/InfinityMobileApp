...
const AttendenceStatus = ({ navigation }) => {
    ...
    const onDateChange = (date, type) => {
        // type is 'END_DATE' if user selected an end date, 'START_DATE' otherwise
        if (type === 'END_DATE') {
            setSelectedEndDate(getParsedDate(date));
        } else {
            setSelectedStartDate(getParsedDate(date));
            setSelectedEndDate(null); // Reset end date when start date changes
        }
    };

    const filterAttendanceRecords = () => {
        setSkipRecord(0); // Reset skipRecord if necessary for your pagination logic
        setSDate(selectedStartDate);
        setEDate(selectedEndDate || selectedStartDate); // Use same date as end date if only one date is selected
