...
const Education = () => {
  ...
  const getEductionData = async () => {
    ...
    const userId = await AsyncStorage.getItem("userId") // Make sure this is the correct type (string or number)
    ...
    try {
      ...
      // Assuming userId is stored in AD_User_ID and is a string. Convert as necessary.
      const filteredData = response.data.records.filter(record => record.AD_User_ID === userId);
  
      // Extract required information from each record
      const educationDetails = filteredData.map(record => ({
        degreeTitle: record.Degree_Title,
