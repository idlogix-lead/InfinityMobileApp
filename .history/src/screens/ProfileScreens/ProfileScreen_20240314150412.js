...
const ProfileScreen = ({ navigation }) => {
  ...
  const generateUserIdentifier = (user) => {
    return `${user.userName}-${user.protocol}-${user.host}-${user.port}-${user.clientId}-${user.clientName}-${user.roleId}-${user.organizationId}-${user.warehouseId}`;
  };
  ...
  const switchAccount = async (identifier) => {
    ...
    console.log(selectedUser,'dd')
    if (selectedUser) {
      const { token, tokenOk, roleId, userName, password, protocol, host, port, clientId, organizationId, warehouseId } = selectedUser;
    ...
    Alert.alert('Confirmation Action', 'Do you want to LogOut?', [
      { text: 'Cancel', style: 'cancel' },
      {
        text: 'Okay', onPress: async () => {
          await AsyncStorage.multiRemove(['currentUserIdentifier', 'userName', 'protocol', 'host', 'port', 'clientId', 'clientName', 'roleId', 'organizationId', 'warehouseId', 'tokenOk', 'token']);
