...
const ProfileScreen = ({ navigation }) => {
  ...
  const switchAccount = async (userName) => {
    // Retrieve current user's data from AsyncStorage
    ...
    const currentUser = { userName: currentUserName, protocol, host, port, clientId, clientName, roleId, organizationId, warehouseId };
    ...
    const areUsersIdentical = (user1, user2) => {
        return user1.protocol === user2.protocol &&
               user1.host === user2.host &&
               user1.port === user2.port &&
               ...
               user1.warehouseId === user2.warehouseId
    };

    // Find a user in the array that matches the current user in all properties
    const selectedUser = usersArray.find(user => areUsersIdentical(user, currentUser));
