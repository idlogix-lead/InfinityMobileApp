...
const ProfileScreen = ({ navigation }) => {
  ...
  const refreshToken = async () => {
    // Assuming you store a refresh token when initially logging in or switching accounts
    ...
    try {
        const response = await fetch(`${protocol}://${host}:${port}/api/v1/auth/refresh`, {
            method: 'POST',
            headers: {
        ...
        return token; // Return the new token for immediate use
    } catch (error) {
        console.error(error);
...
const switchAccount = async (userName) => {
  ...
  if (selectedUser) {
      // Here, you could call refreshToken if your app's logic deems it necessary
      // For demonstration, assuming you always refresh the token when switching accounts
      const newToken = await refreshToken(); // Ensure refreshToken is implemented to handle this logic
      ...
      await AsyncStorage.multiSet([
          ...
          ['token', newToken || selectedUser.token], // Use newToken if refreshed, else fall back to the existing token
      ]);

      navigation.navigate('ProfileScreen'); // Refresh or navigate as needed
  }
};
