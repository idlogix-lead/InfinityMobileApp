...
const ProfileScreen = ({ navigation }) => {
  ...
  const switchAccount = async (identifier) => {
    ...
    console.log(currentUserIdentifier,'loop')
    if (identifier === currentUserIdentifier) {
      bottomSheetRef.current?.close();
      return;
    }
console.log(multiUsers,'ioup')
    ...
    Alert.alert('Confirmation Action', 'Do you want to LogOut?', [
      { text: 'Cancel', style: 'cancel' },
      {
        text: 'Okay', onPress: async () => {
          await AsyncStorage.multiRemove(['currentUserIdentifier', 'userName','password','protocol', 'host', 'port', 'clientId', 'clientName', 'roleId', 'organizationId', 'warehouseId', 'tokenOk', 'token']);
