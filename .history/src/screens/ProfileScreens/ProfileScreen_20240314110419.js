...
const ProfileScreen = ({ navigation }) => {
  ...
  const switchAccount = async (userName) => {
    ...
    console.log("All users in storage:", usersArray); // Log all users for comparison
  
    const isMatch = (user) => {
      const match = user.protocol === protocol &&
                    user.host === host &&
                    user.port === port &&
                    user.userName === userName && // Ensure userName match
                    user.clientId === clientId &&
                    user.clientName === clientName &&
                    user.roleId === roleId &&
                    user.organizationId === organizationId &&
                    user.warehouseId === warehouseId;
      console.log(`Matching ${user.userName}: ${match}`); // Log the match result for each user
      return match;
    };
