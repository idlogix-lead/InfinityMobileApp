import { StyleSheet, Text, View, Dimensions, Image, TouchableOpacity, Alert, ActivityIndicator, BackHandler, StatusBar } from 'react-native'
...
const ProfileScreen = ({ navigation }) => {
  ...
  const funct = async()=>{
    const dataString = await AsyncStorage.getItem('dataString')
  ...
  return (
    ...
    <>
      <View style={{ flex: 1, backgroundColor: 'white', alignItems: 'center' }}>
        ...
        >
          <View>
            <Text style={styles.userAccountTxt}>{userName}</Text>
            <Card handlePress={()=> navigation.navigate('SignIn')}
              Icon={<MaterialCommunityIcons name='plus-circle' color='#877e7e' size={23} />}
              txt='Add Account' />
          </View>
        </RBSheet>
