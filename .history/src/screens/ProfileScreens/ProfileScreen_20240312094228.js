...
const ProfileScreen = ({ navigation }) => {
  ...
  const switchAccount = async (userName) => {
    try {
      // Close the bottom sheet immediately to give feedback to the user
      bottomSheetRef.current?.close();
  
      // Avoid switching if the selected user is already the current user
      const currentUserName = await AsyncStorage.getItem('userName');
      if (userName === currentUserName) {
        console.log('Already on the selected user account.');
        return;
      }
  
      // Fetch the users data from AsyncStorage
      const usersString = await AsyncStorage.getItem('usersData');
      if (!usersString) {
        throw new Error('No users data found in storage.');
      }
  
      const usersArray = JSON.parse(usersString);
      const selectedUser = usersArray.find(user => user.userName === userName);
      if (!selectedUser) {
        throw new Error('Selected user not found in stored data.');
      }
  
      // Set the AsyncStorage with the selected user's details
      const userSettings = [
      ...
      await AsyncStorage.multiSet(userSettings);
  
      // Navigate to the FingerPrintScreen with the necessary parameters
