...
const ProfileScreen = ({ navigation }) => {
  ...
  const getLoginUsers = async () => {
    ...
    const userIdentifiers = usersArray.map(generateUserIdentifier);
  ...
  const switchAccount = async (userName) => {
    ...
    // Define a function to check for a comprehensive match, excluding token and tokenOk
    const isMatch = (user) => {
      return user.protocol === protocol &&
        user.host === host &&
        user.port === port &&
        user.userName === userName && // Still need to match the userName for identification
    ...
    // Find the selected user based on the comprehensive match
    const selectedUser = usersArray.find(isMatch);
  ...
  //   // console.log(usersArray.find(user => user.userName),'dsa')
  ...
  useEffect(() => {
    const getCurrentUser = async () => {
      const currentUser = {
        ...
        // token and tokenOk are excluded from the identifier
      };
      const currentUserIdentifier = generateUserIdentifier(currentUser);
      setSelectedUserName(currentUserIdentifier); // Store the identifier of the current user
    };
  ...
  useEffect(() => {
    switchAccount(selectedUserName);
  }, [selectedUserName]);
        ...
        },
        {
          ...
          onPress: async () => {
            ...
            if (usersDataString) {
              ...
              usersData = usersData.filter(user => user.userName !== currentUserName);
            ...
            <FlatList
              ...
              renderItem={({ item }) => (
                <TouchableOpacity
                  ...
                  onPress={() => {
                    switchAccount(item.userName); // Assuming you want to switch account based on userName
                    setSelectedUserName(generateUserIdentifier(item)); // Update to use the identifier
                  }}
                >
                  <Text style={styles.MultiLogTxt}>{item.userName}</Text> {/* Corrected this line */}
                  <View style={styles.radioButton}>
                    {selectedUserName === generateUserIdentifier(item) && <View style={styles.radioButtonSelected} />}
                  </View>
                </TouchableOpacity>
              )}
            />
