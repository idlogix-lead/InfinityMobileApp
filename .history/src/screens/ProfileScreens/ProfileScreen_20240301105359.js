...
const ProfileScreen = ({ navigation }) => {
  ...
  const switchAccount = async (userName) => {
    ...
    if (selectedUser) {
      await AsyncStorage.setItem('protocol', selectedUser.protocol);
      await AsyncStorage.setItem('host', selectedUser.host);
      await AsyncStorage.setItem('port', selectedUser.port);
      await AsyncStorage.setItem('userName', selectedUser.userName);
      await AsyncStorage.setItem('clientId', selectedUser.clientId);
      await AsyncStorage.setItem('clientName', selectedUser.clientName);
      await AsyncStorage.setItem('roleId', selectedUser.roleId);
      await AsyncStorage.setItem('organizationId', selectedUser.organizationId);
      await AsyncStorage.setItem('warehouseId', selectedUser.warehouseId);
      await AsyncStorage.setItem('tokenOk', selectedUser.tokenOk);
      await AsyncStorage.setItem('token', selectedUser.token);
  
      // Refresh the profile screen or navigate as needed
      navigation.navigate('ProfileScreen'); // Make sure to navigate or refresh the screen to reflect the switched account
    }
  };
            ...
            <FlatList
              ...
              renderItem={({ item, index }) => (
                <TouchableOpacity style={styles.BottomView}>
                  <MaterialCommunityIcons name='account-circle' color='#877e7e' size={27} />
