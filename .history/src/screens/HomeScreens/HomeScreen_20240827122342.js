...
const HomeScreen = ({ navigation, route }) => {
  ...
  const handleGetAttendance = async () => {
    ...
    try {
      ...
      console.log('Raw AttDate:', attendanceRecord.AttDate);
      console.log('Raw AttTime:', attendanceRecord.AttTime);
      
  
        
          // Extract year, month, and day from AttDate
          const [year, month, day] = attendanceRecord.AttDate.split('-').map(Number);
          console.log(typeof(year), month, day)
        
        
          // Extract hour, minute, and second from AttTime
          ...
          // Calculate the difference in hours between the current time and the last attendance time
          const currentTime = new Date();
          const hoursPassed = (currentTime - lastAttDate) / 1000 / 60 / 60;
        
          console.log('Parsed DateTime:', lastAttDate, 'Hours Passed:', hoursPassed);
