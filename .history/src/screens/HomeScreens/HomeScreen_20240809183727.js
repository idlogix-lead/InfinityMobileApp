...
const HomeScreen = ({ navigation, route }) => {
  ...
  const FindBusinessPrtId = async () => {
    ...
    try {
      const response = await axios.get(`${protocol}://${host}:${port}/api/v1/models/AD_User?$filter=AD_User_ID eq ${Id}`,
        ...
        console.log(response?.data?.records[0]?.C_BPartner_ID,'ad')
  ...
  const handleGetAttendance = async () => {
    ...
    try {
      ...
      const filterQuery = `$filter=AD_Client_ID eq ${clientid} and AD_Org_ID eq ${org_id} and C_BPartner_ID eq ${parseInt(partnerId)}`;
