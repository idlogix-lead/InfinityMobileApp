...
const AllTaskScreen = () => {
  ...
  const getAPIData = async () => {
    ...
    fetch(
      `${protocol}://${host}:${port}/api/v1/models/mbl_request_view_v?$filter= SalesRep_ID eq ${Id}`,
      {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
      ...
      .then(data => {
        ...
        const uniqueNames = [
          ...new Set(sortedData.map(item => item.user_name)),
        ];
  ...
  const fetchBasedOnId = async (
    ...
    const promises = idArray.map(id =>
      fetch(
        `${protocol}://${host}:${port}/api/v1/models/R_Request?$filter=SalesRep_ID eq ${id} AND CreatedBy eq ${userId}`,
        {
          method: 'GET',
          headers: {
            'Content-Type': 'application/json',
  ...
  const fetchMonth = async (concatAll, idArray) => {
    ...
    const firstDayOfCurrMonth = moment()
      ...
      .startOf('month')
      .format('YYYY-MM-DD');
    const lastDayOfCurrMonth = moment()
      .subtract('months')
      .endOf('month')
      .format('YYYY-MM-DD');
  ...
  const getReport = async id => {
    ...
    const token = await AsyncStorage.getItem('token');
    fetch(
      `${protocol}://${host}:${port}/api/v1/models/R_Request/${id}/attachments`,
      {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
  ...
  const onSelectedItemsChange = selectedStatusIds => {
    const updatedStatuses = items.filter(item =>
  ...
  useEffect(() => {
    const filtered = data.filter(item => {
      const isNameMatched =
        !assignedName || item?.user_name.includes(assignedName);
      ...
      if (selectedStatus.length > 0) {
        isStatusMatched = selectedStatus.some(status => {
          if (status.value === 'FinalClose') {
            ...
            return item?.R_Status_ID?.identifier
              .toLowerCase()
      ...
      const isStartDateMatched =
        !selectedStartDate || itemStartDate.isSameOrAfter(selectedStartDate);
      const isEndDateMatched =
        !selectedEndDate || itemEndDate.isSameOrBefore(selectedEndDate);
      return (
        isNameMatched &&
        isStatusMatched &&
        isOwnTask &&
      ...
      {!isLoading && (
        <View style={{flex: 1, backgroundColor: 'white'}}>
          <Modal visible={show} animationType="slide" transparent={true}>
            <View style={styles.modalContainer}>
              <View style={styles.modalView}>
                ...
                <TouchableOpacity
                  onPress={() => modalClose('Open', 1000000)}
                  style={styles.txtContainer}>
                  <Text style={styles.txt}>Open</Text>
                </TouchableOpacity>
                <TouchableOpacity
                  onPress={() => modalClose('Waiting', 1000001)}
                  style={styles.txtContainer}>
                  <Text style={styles.txt}>Waiting</Text>
                </TouchableOpacity>
                <TouchableOpacity
                  onPress={() => modalClose('Close', 1000002)}
                  style={styles.txtContainer}>
                  <Text style={styles.txt}>Close</Text>
                </TouchableOpacity>
                <TouchableOpacity
                  onPress={() => modalClose('Final Close', 1000003)}
                  style={styles.txtContainer}>
          ...
          <FlatList
            ...
            renderItem={item => {
              ...
              return (
                <ItemList
                  employName={
                    item.item.user_name ? item.item.user_name : 'My Task'
                  }
          ...
          <TouchableOpacity
            ...
            onPress={() => {
              navigation.navigate('CreateNewReq');
            }}>
                    ...
                    <TouchableOpacity
                      ...
                      onPress={() => {
                        ...
                        onSelectedItemsChange(
                          updatedStatus.map(item => item.id),
                        );
                      }}>
