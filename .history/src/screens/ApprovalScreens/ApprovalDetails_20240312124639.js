...
const ApprovalDetails = ({ route }) => {
    ...
    const shouldRenderView = (paymentDetail) => {
        if (!paymentDetail) return false;
        
        const fieldsToCheck = [
          paymentDetail.C_DocType_ID?.identifier,
          paymentDetail.DocumentNo,
          paymentDetail.DateInvoiced,
          paymentDetail.DateDoc,
          paymentDetail.IsSOTrx,
          paymentDetail.TotalLines,
          paymentDetail.GrandTotal,
        ];
      
        // Check if any of the fields have a value of "No"
        return !fieldsToCheck.some(field => field === 'No');
      };
    ...
    return (
        <View style={{ flex: 1 }} >
            ...
            {paymentDetails.length > 0 && shouldRenderView(paymentDetails[0]) && (
