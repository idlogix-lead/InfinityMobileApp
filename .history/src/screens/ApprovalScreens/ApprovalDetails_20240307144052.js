    ...
    const FetchDetails = async () => {
        try {
            ...
            if (mobelname === 'C_invoice' || mobelname === 'M_Requisition') {
                ...
                const lineURL = `${protocol}://${host}:${port}/api/v1/models/${lineModelName}?$filter=${lineModelName}_ID eq ${recordId}`;
                ...
                setCombinedDetails([...response.data.records, ...lineResponse.data.records]);
            } else {
                setCombinedDetails(response.data.records);
            }
        } catch (error) {
            console.error('Error:', error);
        }
    };
    // console.log(combinedDetails,'d')
