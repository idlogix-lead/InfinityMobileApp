            ...
            <FlatList
                ...
                renderItem={({ item }) => (
                    <ApprovalCard
                        ...
                        Qty={item.PayAmt ? item.PayAmt.toString() : 'null'}
                        Unit={item.C_TenderType_ID?.identifier || ''}
                        Rate={item.TrxType?.identifier || ''}
                        AmtExTax={item.DiscountAmt ? item.DiscountAmt.toString() : ''}
                        Tax={item.TaxAmt ? item.TaxAmt.toString() : ''}
                        TaxAmt={item.WriteOffAmt ? item.WriteOffAmt.toString() : ''}
                        AmtIncTax={item.PayAmt ? item.PayAmt.toString() : ''}
                    />
