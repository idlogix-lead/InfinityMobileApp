...
const ApprovalDetails = ({ route }) => {
    ...
    return (
        ...
        <View style={styles.TopBorderStyle}>
            ...
            {/* <View> */}
                <Text style={styles.TopTxt}>Partner: {paymentDetails[0].C_BPartner_ID.identifier}</Text>
                <Text style={styles.TopTxt}>Document No: {paymentDetails[0].DocumentNo}</Text>
                <Text style={styles.TopTxt}>Date Invoiced: {paymentDetails[0].DateInvoiced}</Text>
                <Text style={styles.TopTxt}>Doc Type: {paymentDetails[0].C_DocType_ID.identifier}</Text>
                <Text style={styles.TopTxt}>Total Lines (Exc Amount): {paymentDetails[0].TotalLines.toFixed(2)}</Text>
                <Text style={styles.TopTxt}>Grand Total (Inc Amount): {paymentDetails[0].GrandTotal.toFixed(2)}</Text>
                <Text style={styles.TopTxt}>{paymentDetails[0].IsSOTrx ? 'Sales Order Transaction' : 'Invoice Vendor'}</Text>
            {/* </View> */}
        </View>
    )}
                ...
                renderItem={({ item }) => {
                    ...
                    return (
                        <ApprovalCard
                            ...
                            Qty={item.QtyInvoiced ? item.QtyInvoiced : ''}
