...
const CreateNewReq = ({ navigation }) => {
  ...
  const saveData = async () => {
    try {
      ...
      // console.log(attachmentData,'4');

      if (attachmentResponse.status !== 204 && attachmentResponse.headers.get("Content-Length") !== "0") {
        try {
          const attachmentData = await attachmentResponse.json();
          console.log(attachmentData, '4');
        } catch (error) {
          console.error('Failed to parse JSON from attachment response', error);
          // Handle parsing error
        }
      } else {
        console.log('No content in attachment response');
  ...
  const getID = async (selectedId, itemName) => {
    ...
    const token = await AsyncStorage.getItem('token')
    try {
      const response = await fetch(`${protocol}://${host}:${port}/api/v1/models/AD_User?$filter=Supervisor_ID eq ${userId}`, {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
  ...
  //     .then(data => {
  //       if (selectedId !== undefined && selectedId !== null) {
  //         setSubOrdinateKey(selectedId); 
  //       } else {
  //         setSubOrdinateKey(null); 
  //       }
  //     })
  //     .catch(error => console.error(error));
  // }
