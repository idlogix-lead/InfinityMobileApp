...
const CreateNewReq = ({ navigation }) => {
  ...
  const selectAssigned = async (selectedItem) => {
    setAssigned(selectedItem.Name); // Assuming 'Name' is the display value you want to show
    setSubOrdinateKey(selectedItem.id); // Directly use the selected item's id
  };
        ...
        <ScrollView style={{ flex: 1 }}>
          <View style={styles.inputContainer}>
            ...
            <View style={{ flexDirection: 'row' }}>
              ...
              <TouchableOpacity
  onPress={() => {
    selectAssigned(item); // Pass the entire item object
...
>
  <FlatList
    data={subdata}
    keyExtractor={item => String(item.id)} // Ensuring item.id is a string for keyExtractor
    renderItem={({ item }) => (
      <TouchableOpacity
        onPress={() => {
          selectAssigned(item); // Ensure 'item' is defined in this scope
          refRBSheet.current.close();
        }}
        style={{ padding: 10, alignItems: 'center' }}
      >
