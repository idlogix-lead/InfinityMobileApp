...
const CreateNewReq = ({ navigation }) => {
  ...
  // console.log(subOrdinateKey)
  // console.log(startDate,'star')
  // console.log(name,'nam')
  // console.log(assigned,'asi')
  // console.log(endDate,'end')
  // console.log(summary,'sumar')
    ...
    try {
      ...
      const requestResponse = await fetch(`${protocol}://${host}:${port}/api/v1/models/R_Request`, {
        ...
        body: JSON.stringify({
          ...
          "PriorityUser": { "id": 'Medium', "identifier": '5', "model-name": "ad_ref_list" },
  ...
  const getID = async (itemIndex, itemValue) => {
    ...
    fetch(`${protocol}://${host}:${port}/api/v1/models/AD_User?$filter=Supervisor_ID eq ${userId}`, {
      ...
      .then(response => console.log(response.json()),'dgfd')
      .then(data => {
        if (itemIndex === 0) {
          setSubOrdinateKey(0)
        } else (
