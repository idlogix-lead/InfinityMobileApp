...
const CustomHeader = ({title,RightIcon,RightPress,style}) => {
    ...
    useEffect(() => {
        const backAction = () => {
            navigation.goBack();
            return true; // Returning true prevents the event from bubbling up & the default back button behaviour
        };

        const backHandler = BackHandler.addEventListener('hardwareBackPress', backAction);

        return () => backHandler.remove(); // Remove the listener on component unmount
