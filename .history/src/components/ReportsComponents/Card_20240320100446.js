...
const Card = ({ item }) => {
    ...
    const [animation2, setAnimation2] = useState(new Animated.Value(1)); 
    const [animation3, setAnimation3] = useState(new Animated.Value(1)); 

    const handleToggle = (expandedState, setExpandedState, animationValue) => {
        let finalValue = expandedState ? 0 : 1; 
