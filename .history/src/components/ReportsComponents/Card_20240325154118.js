...
const Card = ({totalAmount, totalCashAmount,totalBankAmount,title,Header1,Header2,Name,Amount }) => {
                    ...
                    {expanded2 && (
                        <View onLayout={(event) => measureView(event, 'section2')} ref={contentRef2} style={styles.InnerContainer}>
                            {/* {cashItems.map((item, index) => ( */}
                                <View style={styles.CashView} key={index}>
                                    <Text style={styles.InnerTxt}>{Name}</Text>
                                    <Text style={styles.InnerTxt}>{Amount}</Text>
                                </View>
                            {/* ))} */}
                        </View>
                    )}
                        ...
                        <View onLayout={(event) => measureView(event, 'section3')} ref={contentRef3} style={styles.InnerContainer}>
                            {bankItems.map((item, index) => (
                                <View style={styles.CashView} key={index}>
                                    ...
                                    <Text style={styles.InnerTxt}>{item.Amount}</Text>
                                </View>
                            ))}
                        </View>
                    )} */}
