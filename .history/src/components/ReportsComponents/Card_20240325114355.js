                ...
                <View onLayout={(event) => measureView(event, 'section1')} ref={contentRef1}>
                    ...
                    {expanded2 && (
                        <View onLayout={(event) => measureView(event, 'section2')} ref={contentRef2} style={styles.InnerContainer}>
                            {/* which AccountType.id is = B display hear */}
                            {/* {item.filter(i => i.AccountType.id === 'B').map((filteredItem) => (
                                <View key={filteredItem.id}>
                                    <Text>{filteredItem.Name}</Text>
                                    <Text>{filteredItem.Amount}</Text>
                                </View>
                            ))} */}
                        </View>
                    )}
                    ...
                    {expanded3 && (
                        <View onLayout={(event) => measureView(event, 'section3')} ref={contentRef3} style={styles.InnerContainer}>
                            {/* which AccountType.id is = C display hear */}
                            {/* {item.filter(i => i.AccountType.id === 'C').map((filteredItem) => (
                                <View key={filteredItem.id}>
                                    <Text>{filteredItem.Name}</Text>
                                    <Text>{filteredItem.Amount}</Text>
                                </View>
